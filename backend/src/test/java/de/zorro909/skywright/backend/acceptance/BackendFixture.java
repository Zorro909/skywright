...
final class BackendFixture implements AutoCloseable {
	...
	static BackendFixture startWith(Class<?> testConfiguration) {
	...
	private static ConfigurableApplicationContext run(SpringApplicationBuilder builder,
		...
		return builder.properties("skywright.run-commands.enabled=false", "skywright.run-log-capture.enabled=false")

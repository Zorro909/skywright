...
final class HttpProblemDetails {
	...
	private static Metadata metadata(HttpStatusCode status) {
		return switch (status.value()) {
			...
			case 405 ->
				new Metadata("The HTTP method is not supported for this path.", "SKYWRIGHT_HTTP_METHOD_NOT_ALLOWED");
			case 406 -> new Metadata("The requested response representation is not available.",
	...
	private static List<FieldViolation> fieldViolations(Exception exception) {
		...
		return validationFailure.getBindingResult()
			.getFieldErrors()
			.stream()
			.map(fieldError -> new FieldViolation(fieldError.getField(), "INVALID_VALUE",

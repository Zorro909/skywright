...
final class SkyPilotClientFailure extends Exception {
	...
	static SkyPilotClientFailure from(PolyglotException failure) {
		...
		var category = normalized.contains("versionmismatch") || normalized.contains("version mismatch")
				? BridgeFailure.FailureCause.VERSION_MISMATCH
				: normalized.contains("auth") || normalized.contains("unauthorized") || normalized.contains("forbidden")
						? BridgeFailure.FailureCause.AUTHENTICATION

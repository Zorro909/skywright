...
A source outage before admission returns 503 without creating a Run. An outage after commit cannot revoke accepted intent; the response remains 202 with uncertain handoff. Clients should preserve their submission UUID across retries.

## Persistence boundaries

Run Records retain Skywright's accepted definition, task and artifact references. Registry rebinding reads those references through `TrainingProjectArtifactReferences`. Runtime credentials stay in the transient secret channel; only their binding revisions and usage facts are retained.

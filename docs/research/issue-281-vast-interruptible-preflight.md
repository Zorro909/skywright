...
The unchanged SkyPilot 0.13.0 launch path does not meet Skywright's credential isolation contract. Documented configuration can set a bid, but no inspected option prevents the provider key from entering the remote startup command. This is a negative compatibility result, not a live provider qualification. [ADR 0025](../adr/0025-centralize-managed-credentials-in-vault.md) confines provisioning credentials to the SkyPilot API server; [ADR 0023](../adr/0023-gate-the-initial-target-and-registry-matrix.md) requires evidence for each target and purchase mode.
...
Vast supports [scoped API keys and parameter constraints](https://docs.vast.ai/api-reference/permissions). A provisioning key should eventually exclude billing and key administration where possible. Restricted permissions reduce authority, but do not change which role receives the key. A provider-enforced bid constraint is a possible additional safeguard; its exact creation endpoint, parameter handling and rejection behavior have not been qualified. No such policy is claimed to be deployed.

An upstream release that removes both unwanted delivery paths could be evaluated without local SDK edits. Manual rentals or an external provisioning service feeding an SSH pool would be a different integration with new lifecycle, pricing and recovery obligations; they are not implemented workarounds for this issue. No supported, qualified configuration preserving all current contracts was found.

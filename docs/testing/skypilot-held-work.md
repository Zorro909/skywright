...
[Issue #252's measurements](issue-252-cancellation-latency.md) reproduce the first-call deadline crossing with the current runtime and distinguish client-thread CPU work from actual upstream waits. The fixture now requires at least two available CPUs, verifies exactly one startup cancellation, and retains per-request timing and process-exit evidence. Its existing held-work deadlines and warm-up scope are unchanged.

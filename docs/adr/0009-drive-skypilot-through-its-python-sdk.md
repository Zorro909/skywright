...
An Orchestrator Operation is never durable. SkyPilot retains requests for one day, so persisting one would store a pointer that outlives its referent, and ADR 0005 already forbids storing what SkyPilot returns. A backend restart therefore loses the progress view of an in-flight operation but never its effect: correlation is a pure function of the run identity and the derived job name. As clarified by #56, a name is correlation rather than an atomic remote deduplication key. A durable first-dispatch claim owned by #65 authorizes at most one launch. Repeat delivery and restart use observation only; an empty, delayed or unavailable lookup never reauthorizes launch. Transient request IDs remain ephemeral.

...
The control plane owns a durable Target Storage registry. Each registration has a stable Skywright identity, immutable purpose and bucket, qualified non-secret configuration revisions, and role-scoped Credential Binding associations. A successful qualification atomically promotes a candidate configuration; a failed candidate remains repairable and leaves the active configuration untouched. The database stores binding identities and readiness, never credential values.

Run Submission resolves the registration once and materializes its non-secret descriptor into the Run Definition. Direct execution accepts the same descriptor and uses standard AWS credential-provider conventions. Both paths therefore use one storage contract without requiring the SDK to query registry persistence or the control plane to disclose credentials.
...
Repatriation is enabled by default and expressed as a destination Target Storage plus a flag for each Target Class, overridable per Run and materialized into the Run Definition like any other default. Class-specific destinations allow local single-GPU, local multi-GPU, cloud on-demand, and cloud spot Runs to return to different homes without making a provider or purchase mode part of storage identity. Repatriation is a no-op when the execution Target Storage already is the resolved destination, which exempts local runs by arithmetic rather than by a carve-out and covers a deliberately cloud-targeted local run without a special rule. It triggers on terminal states only — finished, failed and aborted, never interrupted, which would move a Run Store out from under its own resume. Failed runs repatriate too, because `O5` and `U10` want diagnosis context long after the cloud store stops being worth paying for.
...
Credential Bindings attach to a Target Storage by consuming role. Credential Projection remains a separate authority: registration records binding identity and readiness, while each Training Process, Transfer Worker, Metric View, or backend operation receives only the role-appropriate provider for its lifetime.

...
The retained table holds what SkyPilot actually said. This decision originally named R3's five states. ADR 0014 adds `cancelled`, so the current six states — waiting, running, interrupted, finished, failed, cancelled — are a pure function over those raw facts, the Run Definition, Execution Attempt and Termination evidence, Recovery Exhaustion Records (ADR 0016), and durable control decisions (ADR 0021), computed per read and never persisted. A derived state is neither SkyPilot-sourced nor Skywright-originated, so storing it would put an interpretation into a table that holds source facts. Because SkyPilot classifies from cluster health rather than cause, the mapping will be wrong somewhere; deriving means a corrected mapping corrects every historical run at once.

R6's automatic infrastructure recovery belongs to SkyPilot. Its recovery behavior is an orchestration fact and does not establish why a Training Process stopped. Ordinary user-error restarts remain disabled with `max_restarts_on_errors: 0`. ADR 0020 supersedes this ADR's original requirement to leave `recover_on_exit_codes` unset: the dedicated finalized-interruption outcome, implemented as 75, is the sole configured exception. That outcome requires a durable checkpoint and Execution Termination Report. Recovery still passes ADR 0016's debt and previous-writer admission checks. Cancellation and ordinary failure remain terminal, and neither a recovery nor an absent report proves preemption.
...
The SkyPilot job is named from the run identity fixed at Run Record creation. Transient request handles and locally interpreted status are never stored; the covered source facts above retain their original source-event identities under #56's correlation contract. Managed-job IDs are database-scoped and renumber when a controller is rebuilt; a derived name does not. Correlation is a pure function of a Skywright-originated fact. The #56 clarification requires an atomic durable first-dispatch claim from #65 rather than treating a name lookup as remote deduplication. Ambiguous delivery is rediscovered without relaunch, and a job whose name does not parse as a run identity is not Skywright's — which lets the reconciler ignore foreign jobs on a shared controller without bookkeeping.

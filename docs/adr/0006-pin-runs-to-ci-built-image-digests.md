...
Skywright supplies that build-and-publication path as a reusable GitHub Action invoked by each Training Project's workflow. The action owns Docker builds, registry authentication, smoke validation, and OCI publication, and may consume the public contract-compilation interfaces from its pinned Skywright SDK revision. The installable Python SDK remains a runtime and contract-authoring library: it contains no Training Project image-publishing command, registry transport, or publication orchestration.

## Resolution and failure

A Run Definition pins the whole digest map rather than one image, because `T1` and `T3` express a target as capabilities and the accelerator vendor is not known until dispatch. SkyPilot selects `image_id` per candidate, and the digest actually used derives from the infrastructure it selected (ADR 0005). A version may declare a subset of backends; submission fails validation when the requested capabilities could only be met by a backend that version was not built for, which raises `T6`'s explicit failure before anything provisions. The backend enumerates registry display metadata — the provenance label paired with its immutable manifest digest — on demand and resolves contracts and images only for the version selected. Both operations return immutable, age-bearing results so callers can cache them without losing their observation time; a failed live enumeration reports the registry as unreachable instead of substituting stale tags. Cloning verifies resolvability first and fails with a reason rather than dying on the instance.

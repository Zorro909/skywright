...
The resolved Run Configuration is exported into the event directory as TensorBoard HParams metadata at run start. It is written once, never read back, and the Run Definition remains authoritative. The export makes configuration visible when a metric directory is detached from the backend, whether during a post-mortem or when several runs' metrics are downloaded to be compared by hand; it does not make the directory semantically self-contained, because TensorBoard events omit the Metric Definitions and ADR 0015 chooses not to copy their catalog into the Run Store.

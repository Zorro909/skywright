...
Skywright owns metric persistence and binds to no experiment tracker. A run's metrics are TensorBoard event files written by the Run Context into that run's own Run Store, under a `metrics/` prefix beside its checkpoints, samples, and artifacts. ADR 0001 already fixed the event encoding; this decision fixes the durable home, the write protocol, the read path, and the visualization surface. Skywright stores no metric value anywhere else — not in its database, not in a tracker — so the Run Store is the source `Q5` demands be *determined* for the metrics of completed runs rather than casually persisted in the backend, and `Q1` is satisfied structurally rather than by argument. [ADR 0015](0015-compose-version-pinned-metric-contracts.md) separately fixes metric semantics and deliberately stores no resolved catalog beside these values.
...
No portable append exists: S3 `PutObject` replaces whole objects, multipart parts are unreadable until completion, and the append extensions that do exist are not something an S3-compatible bucket generally offers. The open event segment is re-`PUT` on each flush and sealed after the configured event count, after which a new segment starts in the same directory. File count stays proportional to the number of events rather than the number of flushes, and no upload grows with the run's whole history. The Run Configuration sets `metrics.flushInterval` to 10 seconds and `metrics.segmentRoll` to 1,000 events by default. Both values may change on recovery.
...
The resolved Run Configuration is exported once into the event stream as one TensorBoard text summary containing its complete canonical JSON document. HParams metadata cannot represent arbitrary nested JSON, arrays, `null`, or every lossless number without coercion, so Skywright does not flatten the configuration into HParams. The export is never read back, and the Run Definition remains authoritative. It makes configuration visible when a metric directory is detached from the backend, whether during a post-mortem or when several runs' metrics are downloaded for manual comparison. The directory is not semantically self-contained because TensorBoard events omit the Metric Definitions, and ADR 0015 chooses not to copy their catalog into the Run Store.

`U1` needs a run's progress without a metric index existing anywhere. The training process therefore writes a small Progress Record into its Run Store, overwritten each flush, holding the current Step, latest Durable Safe Point, optional target Step, and write time. The target is absent unless a future project contract supplies one because stopping criteria are project-owned and may be dynamic. Readers show an unknown target without inventing a completion percentage. The record is Skywright-originated with no other source, and its timestamp makes it the aged intermediate result `Q2` requires. It is one current value per run rather than a series, so no second copy of metric data is created.

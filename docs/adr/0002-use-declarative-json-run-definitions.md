...
Each Training Project Version must carry a version-bound project schema contribution, defaults document, Defaults Completion Witness, Project Metric Contract, and the exact Skywright schema identities against which CI validated them. The backend independently composes and validates both contracts. JSON Schema's `default` annotation is not relied upon to mutate instances; Skywright deterministically combines library defaults, version defaults and a submission's overrides, validates the resolved configuration, and persists all resulting values explicitly. [ADR 0012](0012-compose-one-owned-configuration-tree.md) defines configuration ownership and precedence; [ADR 0013](0013-resolve-configuration-with-structural-overlays.md) defines exact merge, `null`, completeness and conformance semantics. The exact mechanism used to pin a Training Project Version remains a downstream decision.

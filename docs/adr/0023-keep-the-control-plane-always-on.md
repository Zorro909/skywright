...
- Losing the database removes the Run index and definitions, cost derivation, durable decisions, and every dependent read or control. SkyPilot execution and training-process writes may continue; the backend never reconstructs authority from stale data.

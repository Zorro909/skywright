...
The prototype has two target adapters behind this module: local AMD Kubernetes and Vast.ai. Vast purchase-mode selection is explicit: on-demand is preferred, with interruptible included by the owner’s 2026-09-12 decision when needed for enforceable launch pricing. Each mode has independent evidence gates. Their target and image-pull behaviors differ, while Run identity, configuration, storage, credential, idempotency and lifecycle rules remain shared. Skywright does not add a provider plugin system, generic provider option maps or separate configuration authority. A later target earns another adapter only when its qualified behavior differs from the existing adapters.

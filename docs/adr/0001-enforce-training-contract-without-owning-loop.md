...
- Projects register all standard and project-specific Checkpoint State before training. The library owns serialization, restoration, atomic publication, and retention. For recovery, project-specific completeness is validated and registered state is restored at `context.start()`, after registration and before the training loop. Admission and storage checks remain before Training Project entry; the owner confirmed this boundary on 2026-09-06 for #52.

...
try:
    ...
    if sys.argv[2] == "true":
        if "IS_SKYPILOT_JOB_CONTROLLER" in os.environ or not utils.is_consolidation_mode(): raise ValueError()
        if not Path("~/.sky/.jobs_controller_consolidation_reloaded_signal").expanduser().exists(): raise ValueError()
...
'''
    result = kube.run("exec", "-i", "deployment/skywright-skypilot-api-server", "-n", "skywright",
                      ...
                      str(settings.get("vastProvider") is not None).lower(),

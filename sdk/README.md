...
`tests/integration/dataset/test_dataset_access_system.py` reads frozen corpora generated by the original
...
Pytest discovers `tests/unit/` recursively by default. New unmarked `test_*.py`
modules there run in `scripts/check`, including MDS decoding. Put service tests in
`tests/integration/`, and tests requiring the optional Dataset stack in its
`dataset/` subdirectory. `scripts/integration` discovers the service directories
separately, installing the Dataset extra only for the Dataset group. The other
suites exclude that directory before importing any modules.

Tests under `tests/system/` run against built artifacts through `scripts/verify`.
The directory hooks add `integration`, `dataset` and `system` markers for focused
selection. Unknown markers fail immediately. Artifact fixtures require a wheel
when they execute, so collection needs no wheel. A collection regression creates
future modules, including one with an unavailable optional import, and verifies
that each suite discovers only its intended modules.

To run a specific real-service case, select its directory explicitly:

```bash
uv run --locked --group ml-test pytest tests/integration \
  --ignore=tests/integration/dataset -k checkpoint_recovery
```

...
Install `skywright[dataset]` in a Python 3.10 through 3.13 environment. MosaicML Streaming 0.13
requires NumPy below 2.2; this extra pins torchvision 0.27, matching PyTorch 2.12.
The base SDK still supports Python 3.14, but Dataset reads require the older Python
range until MosaicML supports a compatible NumPy release. Environment Profile
assembly and GPU qualification remain in #231 and #235.

Runtime assembly creates `skywright.dataset.MdsDatasetAccess` from a pinned
`DatasetDefinition` and a selected `DatasetLocation`, then passes it to
...
epoch boundaries. This initial batch cursor assumes one batch per committed Step;
#45 owns exact continuation across arbitrary Step regrouping and recovery.

`tests/test_dataset_access_system.py` writes real MDS corpora and reads them through
S3 against pinned SeaweedFS. It checks hits, eviction, restart, location change,
cache loss, digest rejection and direct Training Process execution with file
credentials. `scripts/integration` runs this suite in an isolated Python 3.13
environment and writes I/O evidence under `target/dataset-read-measurements/`.
